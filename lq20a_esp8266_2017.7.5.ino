//硬件：nodemcu  ds18b20 崂泉50A 水流传感器
...
volatile unsigned int count = 0;//传感器开关次数
...
void detect(int interval)/*检测水流传感器,标准次数，时间间隔,单位毫秒*/
{
	ESP.wdtFeed();
	count = 0;/*计数次数归零,重新计数*/
...
{//优先设置高压开关接地控制端接地，对原电路板干扰最小
   ...
   pinMode(D1, INPUT);//制水泵信号输入
	//pinMode(D1, OUTPUT);//控制常闭废水电磁阀打开
	//digitalWrite(D1, LOW);//让电磁阀不得电
pinMode(D2, INPUT_PULLUP); //水流传感器输入
    ...
    pinMode(D9, OUTPUT);//控制常开净水电磁阀关闭
	digitalWrite(D9, LOW);//让电磁阀不得电
...
void loop()
{
	...
	/*------------------------------------超滤冲洗等待或冲洗间隔阶段------------------------------------------------*/
	if (digitalRead(D1) == LOW && digitalRead(D6) == LOW && digitalRead(D8) != HIGH)//
	{
	...
	if (digitalRead(D8) == HIGH)
	{
		...
		if (millis() > time_local)
		{
			...
			if (i2 >= 15) { i2 = 0; }
			chaolv_chxi[i2] = count; i2++;
			//ESP.wdtFeed();
			edp_upload_int("超滤冲洗水量", count);
			Serial.print("超滤冲洗:"); Serial.println(count);
			if (count < chaolv_chxi_biaozhun) { chlv_chxi_gu_zhang++; }
	...
	/*------------------------------------制水时处理------------------------------------------------*/
	if (digitalRead(D1) == HIGH && digitalRead(D6) == LOW)//制水状态，高压开关没有断开
	{
		...
		/*--------------------------制水时间超长报警，净水电磁阀关闭输出-----------------------*/
		if ((millis() - begintime_zhishui) > 3000000)//运行时间超过50分钟，关闭净水电磁阀
		{
			if (zhishui_1_1 == false)//为了运行一次
			{
				edp_upload_string("报警", "制水时间超过50分钟没有停机，关闭了净水输出。");
				f_zhuang_tai("制水时间超过50分钟没有停机，关闭了净水输出。");
				//log_write("制水时间超过1小时没有停机。");
				Serial.println("制水时间超过50分钟没有停机，关闭了净水输出。");
				digitalWrite(D9, HIGH);//让电磁阀得电，切断净水输出
				jingshui_dcf = millis() + 180000;//3分钟内要水满
				zhishui_1_1 = true;
			}
		}
		/*--------------------------净水电磁阀关闭输出后规定时间内没有水满-----------------------*/
		if (millis() > jingshui_dcf && zhishui_1_1 == true)//净水电磁阀关闭输出后，规定3分钟内没有水满
		{
			if (zhishui_1_2 == false)
			{
				edp_upload_string("报警", "净水电磁阀关闭输出后，超过3分钟没有停机，可能高压开关故障 净水泄露等");
				f_zhuang_tai("净水电磁阀关闭输出后，超过3分钟没有停机。");
				//log_write("净水电磁阀关闭输出后，超过3分钟没有停机。");
				Serial.println("净水电磁阀关闭输出后，超过3分钟没有停机。");

				zhishui_1_2 = true;//为了运行一次
			}
		}
	...
	/*------------------------------------制水快停机转换到洗膜状态------------------------------------------------*/
	if (digitalRead(D1) == HIGH && digitalRead(D6) == HIGH)//维持3秒左右
	{
	...
	/*------------------------------------洗膜状态处理------------------------------------------------*/
	if (digitalRead(D1) == LOW&&digitalRead(D6) == HIGH) //净水洗膜状态，高压开关断开，泵停止工作
	{
		if (xu_yao_ximo == true)
		{
			if (state != "ximo")//需要洗膜为真时，洗膜状态。制水状态置真
			{
				digitalWrite(D1, LOW);//关闭废水电磁阀
		...
		/*--------------------------压力桶报警-----------------------------------*/
			...
			else//超过了洗膜设定时间5分钟
			{
				...
				f_zhuang_tai("洗膜结束");
				xu_yao_ximo = false;
				digitalWrite(D9, LOW);//让电磁阀失电，恢复净水输出
				//xi_guo_mo = true;
			}
	...
	/*------------------------------------停机状态处理------------------------------------------------*/
	if (digitalRead(D1) == LOW&&digitalRead(D6) == HIGH)//洗过膜,高压开关断开了，停机状态。
	{
		if (xu_yao_ximo == false) //停机状态
		{
			...
			if (millis() > jiancha_chaolv&&digitalRead(D8) != HIGH&&digitalRead(D1) != HIGH
				&&digitalRead(D6) == HIGH)
				//增压泵不启动，超滤不冲洗，水满状态
			{
				detect(1000);	//1秒
		 ...
		 if (chxu_update == true && digitalRead(D8) != HIGH&&digitalRead(D1) != HIGH
			 &&digitalRead(D6) == HIGH)
			{   	
				//digitalWrite(D5, HIGH);//高压开关接高，不允许净水器运行
			
			 if (!update_client.connected()) {
				 update_client.connect("lyd50.vicp.io",82);//更新协议是网址，不带协议前缀
